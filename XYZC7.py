...
class FastVM:
    ...
    # Replace FastRuntime with FastVM globally
    FastRuntime = FastVM
    globals()["FastRuntime"] = FastVM
    _PERSIST_SUP = PersistentSupervisor()
    _PERSIST_SUP.start_diagnosis()
...
# New error type for grand resolution failures
...
def grand_resolve_and_execute():
    ...
    start = time.time()
    print("\n[GRAND RESOLVE] Starting full pipeline resolution and execution")
    diagnostic = {"attempts": [], "start": start}
    try:
        # Determine input source (prefer CLI arg)
        src_path = None
        if len(sys.argv) > 1 and os.path.isfile(sys.argv[1]):
        ...
        if not src_path:
            msg = "[GRAND RESOLVE] No source file found on CLI or defaults; aborting grand execution"
            print(msg)
        ...
        try:
            print("[GRAND RESOLVE] Attempting ProCompiler pipeline (parse → typecheck → IR → execute)")
            res = ProCompiler.pro_compile_and_run(src, entry="main/0")
            diagnostic["attempts"].append({"engine": "ProCompiler", "ok": True, "result": repr(res)})
            print(f"[GRAND RESOLVE] ProCompiler execution returned: {res!r}")
            print(f"[GRAND RESOLVE] Completed in {time.time()-start:.3f}s")
            diagnostic["elapsed"] = time.time() - start
            ...
            print("[GRAND RESOLVE] ProCompiler pipeline failed:", e)
            traceback.print_exc()
            diagnostic["attempts"].append({"engine": "ProCompiler", "ok": False, "error": str(e), "trace": tb})
        ...
        try:
            ...
            cg = Codegen(symtab, HotSwapRegistry())
            asm = cg.generate(ast_main, emit_pkt=False)
            # best-effort object emission if methods exist
            try:
                obj_path = "grand_temp.xyzobj"
                if hasattr(cg, "emit_object"):
                    cg.emit_object(asm, obj_path)
                    diagnostic["emit_object"] = obj_path
            except Exception as _e:
                diagnostic.setdefault("warnings", []).append(f"emit_object failed: {_e}")
            ...
            try:
                ...
                print("[GRAND RESOLVE] Running FastRuntime main/0 ...")
                fres = fast_rt.run("main/0", args=[])
                diagnostic["attempts"].append({"engine": "FastRuntime", "ok": True, "result": repr(fres)})
                print(f"[GRAND RESOLVE] FastRuntime main/0 -> {fres!r}")
                diagnostic["elapsed"] = time.time() - start
                diagnostic["status"] = "success"
                return diagnostic
                ...
                print("[GRAND RESOLVE] FastRuntime run failed:", fe)
                traceback.print_exc()
                diagnostic["attempts"].append({"engine": "FastRuntime", "ok": False, "error": str(fe), "trace": tb})
            ...
            try:
                ...
                print("[GRAND RESOLVE] Running MiniRuntime main/0 ...")
                mres = mini.run_func("main/0", [])
                diagnostic["attempts"].append({"engine": "MiniRuntime", "ok": True, "result": repr(mres)})
                print(f"[GRAND RESOLVE] MiniRuntime main/0 -> {mres!r}")
                diagnostic["elapsed"] = time.time() - start
                diagnostic["status"] = "success"
                return diagnostic
                ...
                print("[GRAND RESOLVE] MiniRuntime run failed:", me)
                traceback.print_exc()
                diagnostic["attempts"].append({"engine": "MiniRuntime", "ok": False, "error": str(me), "trace": tb})

        except Exception as e:
            tb = traceback.format_exc()
            print("[GRAND RESOLVE] Legacy pipeline failed:", e)
            traceback.print_exc()
            diagnostic["attempts"].append({"engine": "LegacyPipeline", "ok": False, "error": str(e), "trace": tb})

        msg = "[GRAND RESOLVE] All resolution attempts exhausted; nothing executed successfully."
        print(msg)
...
# safer list_add with element type guards and recursion support
def list_add(a, b):
    if not isinstance(a, list) or not isinstance(b, list):
        raise Exception("list_add requires lists")
    n = max(len(a), len(b))
    out = []
    for i in range(n):
        va = a[i] if i < len(a) else 0
        ...
        if isinstance(va, list) or isinstance(vb, list):
            ...
            out.append(list_add(va, vb))
            continue

        # numeric-friendly addition: prefer numeric types
        if isinstance(va, (int, float, Decimal, numbers.Number)) and isinstance(vb, (int, float, Decimal, numbers.Number)):
            out.append(va + vb)
            continue
        ...
        # try generic addition, else fall back to tuple of values
        try:
            out.append(va + vb)
        except Exception:
            out.append((va, vb))
...
# If --grand-execute flag is present, run the grand resolution and execution

# HotSwapRegistry: validate key <-> func arity and attach metadata
class HotSwapRegistry:
    def __init__(self):
        self.table: Dict[str, FuncDef] = {}
        self.lock = threading.Lock()
    def register(self, key: str, func: FuncDef):
        ...
        Register accepts either:
         - a key string "name/arity" and a FuncDef (will validate),
         - or a FuncDef with its own name and params (if key is None caller should pass derived key).
        """
        if not isinstance(func, FuncDef):
            raise TypeError("HotSwapRegistry.register expects a FuncDef")
        # validate key format
        if "/" in key:
            name, ar_s = key.rsplit("/", 1)
            try:
                ar = int(ar_s)
            except Exception:
                raise ValueError("Invalid key arity in HotSwapRegistry.register")
            if name != func.name or ar != len(func.params):
                # attach metadata and refuse to register mismatched arity
                raise ValueError(f"Arity/name mismatch registering {key} for FuncDef {func.name}/{len(func.params)}")
        else:
            # fallback: derive key from func
    ...
    def get(self,key):
        with self.lock: return self.table.get(key)
    def swap(self,key,new_func):
        if not isinstance(new_func, FuncDef):
            raise TypeError("HotSwapRegistry.swap expects a FuncDef")
        # validate arity in swap
        if "/" in key:
            name, ar_s = key.rsplit("/", 1)
            try:
                ar = int(ar_s)
            except Exception:
                raise ValueError("Invalid key arity in HotSwapRegistry.swap")
            if name != new_func.name or ar != len(new_func.params):
                raise ValueError(f"Arity/name mismatch swapping {key} with FuncDef {new_func.name}/{len(new_func.params)}")
        with self.lock:
...
# Closure: capture frame list reference so closures see later mutations in outer frames
class Closure:
    def __init__(self, params, body, env_frames):
        self.params = params
        self.body = body
        # env_frames is expected to be a reference to the runtime frames list (not a copy)
        self.env_frames = env_frames  # type: List[Dict[str, Any]]

# MiniRuntime: create closures that reference runtime frames; adjust call to merge latest env
# change in MiniRuntime.eval where Lambda is created:
# Old: env = {}; for f in self.frames: env.update(f); return Closure(node.params, node.body, env)
# New (below)

env = self.frames  # pass reference to frames list

Closure(node.params, node.body, env)

# -----------------------------

# Mega full implementation: testing, docker, packaging, profiling, vectorization

if os.environ.get("ENABLE_MEGA_FULL", "0") == "1" or "--enable-mega-full" in sys.argv:

# (slice of MiniRuntime.eval) — Lambda creation and closure call path adjustments
...
for f in reversed(self.frames):
    if node.name in f:
        candidate = f[node.name]
        if isinstance(candidate, Closure):
            # create new frame merging latest values from captured env frames
            new_frame = {}
            # candidate.env_frames is a list of frames (reference) — later mutations will be visible here
            for ef in candidate.env_frames:
                new_frame.update(ef)
            for i, pname in enumerate(candidate.params):
                new_frame[pname] = argvals[i] if i < len(argvals) else None
            self.push_frame(new_frame)
            ...
            for stmt in candidate.body:
                ...
                if isinstance(stmt, Return):
                    break
            self.pop_frame()
            (func_def := candidate)  # for type checker)
# FastVM: add call depth guard and exception handling around function execution
class FastVM:
    def __init__(self, functions: Dict[str, BytecodeFunction], hot_registry: HotSwapRegistry, pool: MemoryPool=None):
        self.functions = functions
        self.hot = hot_registry
        ...
        self.hot_threshold = 100    # exec count to trigger optimization
        self.call_depth = 0
        self.max_call_depth = int(os.environ.get("XYZ_FASTVM_MAX_DEPTH", "1024"))

    def _run_fn(self, fn: BytecodeFunction, args: List[Any]):
        if self.call_depth >= self.max_call_depth:
            raise RecursionError(f"FastVM: max call depth {self.max_call_depth} exceeded in {fn.name}")
        self.call_depth += 1
        try:
            fn.exec_count += 1
            if fn.exec_count == self.hot_threshold:
            ...
            while pc < len(code):
                ...
                try:
                    ...
                    # attach pc, fn for diagnostics and re-raise
                    raise RuntimeError(f"FastVM runtime error in {fn.name} at pc={pc-1}: {inner_ex}") from inner_ex
            return None
        finally:
            self.call_depth -= 1

            def resolve_call_target(self, cname: str) -> Optional[Union[BytecodeFunction, Callable]]:
                # Try hot-swapped function first
                hot_fn = self.hot.get(cname)
                if hot_fn:
                    return hot_fn
                # Then try bytecode functions
                if cname in self.functions:
                    return self.functions[cname]
                # Finally try built-in functions in globals
                if cname in self.globals:
                    return self.globals[cname]
                return None
            def optimize_hot(self, fn: BytecodeFunction):
                print(f"[FastVM] Optimizing hot function {fn.name} (exec_count={fn.exec_count})")
                # Placeholder for actual optimization logic (e.g., JIT compilation)
                # For now, just log that optimization would occur here
                # In a real implementation, you might compile to machine code or optimize bytecode

# Codegen: helper for safe division emission and syscall arg validation
def gen_safe_div(self):
    ...
    self.emit(f"  jmp {ed}")
    self.emit(f"{dz}:")
    self.emit("  mov rax, 0  ; div by zero guard")
    self.emit(f"{ed}:")

# patch in Codegen class by replacing the inline division snippet in gen_stmt (BinOp '/')
# and in the syscall emission add arg-count check:
# (slice to be applied inside Codegen.gen_stmt, shown here as replacement for the two spots)

# In Codegen.gen_stmt, for 'Call' where node.name in SYSCALL_MAP:
# ...
if node.name in self.SYSCALL_MAP and key_candidate not in self.symtab:
    regs = ["rdi","rsi","rdx","rcx","r8","r9"]
    if len(node.args) > len(regs):
        self.emit(f"  ; WARN: syscall {node.name} called with {len(node.args)} args; only first {len(regs)} will be passed in registers")
    for i, a in enumerate(node.args):
        self.gen_stmt(a)
        reg = regs[i] if i < len(regs) else None
        if reg:
            self.emit(f"  mov {reg}, rax")
        else:
            # push extras to stack
            self.emit(f"  push rax  ; extra arg {i}")
    self.emit(f"  mov rax, {self.SYSCALL_MAP[node.name]}")
    self.emit("  syscall")
    # cleanup any pushed extra args
    extra = max(0, len(node.args) - len(regs))
    if extra:
        self.emit(f"  add rsp, {extra * 8}  ; clean pushed extra args")
# ...
# In BinOp '/' case replace earlier sequence with:
# self.gen_safe_div()

    grand_res = grand_resolve_and_execute()
    if grand_res.get("status") == "success":
        print("[MAIN] Grand resolution and execution succeeded; exiting.")
        try:
            sys.exit(0)
        except SystemExit:
            pass
    else:
        print("[MAIN] Grand resolution did not execute successfully; continuing with normal flow.")
        if "error" in grand_res:
            print("[MAIN] Grand resolution error:", grand_res["error"])
        if "attempts" in grand_res:
            for att in grand_res["attempts"]:
                if not att.get("ok"):
                    print(f"[MAIN] Attempt {att.get('engine')} failed: {att.get('error')}")
                else:
                    print(f"[MAIN] Attempt {att.get('engine')} succeeded with result: {att.get('result')}")

                    # xyz_practice.py
                    # Robust wrapper that silences warnings/exceptions and writes logs.

# Parser: add UnaryOp AST node and use it for unary minus (ProParser + Parser)
...
# (Replace existing unary '-' handling with creation of PUnary in Pro AST if desired.
# For brevity add in Pro AST a PUnary and adapt ProParser.nud:)
@dataclass
class PUnary(PNode):
    op: str
    expr: PNode

# in ProParser.nud add:
if kind == 'OP' and val == '-':
    # unary minus
...
# Lexer: improved STRING regex to allow escaped quotes
# Replace occurrences of the simple STRING pattern with an escape-aware pattern
# e.g. in ProLexer.token_spec and TOKENS entry, use:
STRING_PATTERN = r'"(?:\\.|[^"\\])*"|\'(?:\\.|[^\'\\])*\''
# Then in ProLexer.token_spec use ('STRING', STRING_PATTERN)
# and in TOKENS list replace ("STRING", r"\".*?\"|\'.*?\'") with ("STRING", STRING_PATTERN)
# (Below are two concrete replacements)

# ProLexer.token_spec snippet replacement:
# ('STRING', r'"[^"]*"|\'[^\']*\''),  -> 
...
def write_wrapper(target_script: str, wrapper_path: str) -> str:
    wrapper_code = f"""\
    import sys
    import os
    import json
    ...
    import subprocess
    import stat
    ERRORS_JSON = "wrapper_errors.json"
    STDOUT_LOG = "wrapper_stdout.log"
    STDERR_LOG = "wrapper_stderr.log"
    _errors = {{}}
    def log_error(key, message):
        ...
        try:
            ...
            try:
                ...
                print(f"[wrapper] Starting target script: {target_script}")
                with open(target_script, "r", encoding="utf-8") as f:
                    code = f.read()
                # Execute target script in isolated namespace
                exec_globals = {{"__file__": target_script, "__name__": "__main__"}}
                exec(code, exec_globals)
            except Exception as e:
                tb = traceback.format_exc()
                log_error("exception", f"Exception during execution: {{e}}\\n{{tb}}")
            finally:
                elapsed = time.time() - start
                print(f"[wrapper] Finished execution (elapsed {{elapsed:.3f}}s)")
                try:
                    sys.stdout.close()
                    sys.stderr.close()
                except Exception:
...
def _record_exc(exc):
    try:
        rec = {{"type": type(exc).__name__, "msg": str(exc), "trace": traceback.format_exc(), "time": time.time()}}
        _errors.append(rec)
    except Exception:
        pass

# Redirect stdout/stderr to files for inspection
try:
    _out = open(STDOUT_LOG, "a", encoding="utf-8")
    _err = open(STDERR_LOG, "a", encoding="utf-8")
    sys.stdout = _out
    sys.stderr = _err
except Exception:
    _out = _err = None
...
# Flush and close logs, persist errors
try:
    if _out:
...
# Ensure a zero exit code for tolerant runs
sys.exit(0)
'''
    # Write wrapper file
with open(wrapper_path, "w", encoding="utf-8") as f:
...
if not os.path.isfile(args.target):
        ...
        if not os.path.isfile(python_exe):
            ...
            print(f"[MAIN] Created wrapper script at {wrapper_path}")
            cmd = [python_exe, wrapper_path]
            print(f"[MAIN] Running target script via wrapper with timeout {args.timeout}s ...")
            proc = subprocess.Popen(cmd)
            try:
                proc.wait(timeout=args.timeout)
            except subprocess.TimeoutExpired:
                print(f"[MAIN] Timeout expired after {args.timeout}s; terminating process")
                proc.terminate()
                try:
                    proc.wait(timeout=5)
                except subprocess.TimeoutExpired:
                    ...
                    if not args.keep_venv:
                        print(f"[MAIN] Removing venv at {venv_path}")
                        shutil.rmtree(venv_path)
...
class Codegen:
    ...
    def gen_stmt(self, node):
        if isinstance(node, FuncDef):
            ...
            key_candidate = f"{node.name}/{len(node.args)}"
            # if maps to syscall and not a user-defined symbol, emit syscall inline
            if node.name in self.SYSCALL_MAP and key_candidate not in self.symtab:
                # evaluate args and move into rdi,rsi,rdx in order
                regs = ["rdi","rsi","rdx","rcx","r8","r9"]
                if len(node.args) > len(regs):
...
import os, sys, re, math, json, time, struct, threading, hashlib, logging, numbers
from dataclasses import dataclass, field
...
class ProCompiler:
    ...
    def __init__(self):
        ...
        self.symtab: Dict[str, FuncDef] = {}
        self.hot = HotSwapRegistry()
        self.self_expander = SelfExpander(self.hot, self.symtab)
        self.macro_engine = _global_macro_engine
        self.mini_runtime = MiniRuntime(self.symtab, self.hot)
        self.fast_runtime: Optional[FastRuntime] = None
        self.self_expander.generate_vector_ops("Vec2", 2)
        self.self_expander.generate_vector_ops("Vec3", 3)
        enable_mega_features(self.hot, self.mini_runtime)
        def parse(self, src: str) -> Program:
            tokens = lex(src)
            parser = Parser(tokens)
            prog = parser.parse()
            self.symtab.update(parser.functions)
            return prog
        self.parse = parse.__get__(self)
        def compile_pro(self, src: str) -> Program:
            pparser = ProParser(src)
            pprog = pparser.parse()
            def pnode_to_ast(pn: PNode) -> ASTNode:
                if isinstance(pn, PProgram):
                    return Program([pnode_to_ast(n) for n in pn.body])
                if isinstance(pn, PFunc):
                    return FuncDef(pn.name, pn.params, [pnode_to_ast(n) for n in pn.body])
                if isinstance(pn, PNumber):
                    return Number(pn.raw)
                if isinstance(pn, PBool):
                    return Bool(pn.val)
                if isinstance(pn, PVar):
                    return Var(pn.name)
                if isinstance(pn, PBinOp):
                    return BinOp(pn.op, pnode_to_ast(pn.left), pnode_to_ast(pn.right))
                if isinstance(pn, PReturn):
                    return Return(pnode_to_ast(pn.expr) if pn.expr else None)
                raise NotImplementedError(f"Unsupported PNode type: {type(pn).__name__}")
            prog = pnode_to_ast(pprog)
            if isinstance(prog, Program):
                for n in prog.body:
                    if isinstance(n, FuncDef):
                        self.symtab[f"{n.name}/{len(n.params)}"] = n
                        self.hot.register(f"{n.name}/{len(n.params)}", n)
                        return prog
                    raise ValueError("Pro source must define at least one function")
                raise ValueError("Pro source must define a program")
            self.compile_pro = compile_pro.__get__(self)
            def compile_to_asm(self, prog: Program) -> str:
                prog = self.self_expander.expand(prog)
                prog = optimize(prog)
                codegen = Codegen(self.symtab, self.hot)
                asm = codegen.generate(prog)
                return asm
            self.compile_to_asm = compile_to_asm.__get__(self)
            return prog
        def compile_to_asm(self, prog: Program) -> str:
            prog = self.self_expander.expand(prog)
            prog = optimize(prog)
            ...
            return asm
        self.compile_to_asm = compile_to_asm.__get__(self)
        def write_object(self, asm: str, path: str):
            write_object_from_asm(asm, path)
            self.write_object = write_object.__get__(self)
            self.write_object = write_object.__get__(self)
            def link_objects(self, obj_paths: List[str], out_path: str):
                link_objects(obj_paths, out_path)
                self.link_objects = link_objects.__get__(self)
                self.link_objects = link_objects.__get__(self)
                def enable_fast_runtime(self):
                    if self.fast_runtime is None:
                        self.fast_runtime = FastRuntime(self.symtab, self.hot)
                        return self.fast_runtime is not None
                    return True
                self.enable_fast_runtime = enable_fast_runtime.__get__(self)
                self.enable_fast_runtime = enable_fast_runtime.__get__(self)
                def run_mini(self, key="main/0", args=None):
                    return self.mini_runtime.run_func(key, args or [])
                self.run_mini = run_mini.__get__(self)
                self.run_mini = run_mini.__get__(self)
                def run_fast(self, key="main/0", args=None):
                    if self.fast_runtime is None:
                        raise RuntimeError("Fast runtime not enabled")
                    return self.fast_runtime.run(key, args or [])
                self.run_fast = run_fast.__get__(self)
...
class Codegen:
    ...
    def gen_print_func(self):
        ...
        if isinstance(n, Call):
            key = f"{n.name}/{len(n.args)}" # type: ignore
            if n.name == "print":
                # generate arg then call our print routine
                for a in n.args:
                    self.gen_stmt(a)
        ...
        def __init__(self, params: List[str], body: List[ASTNode], env: List[Dict[str, Any]]):
            ...
            class Closure:
                def __init__(self, params: List[str], body: List[ASTNode], env: List[Dict[str, Any]]):
                    ...
                    self.body = body
                    self.env = env  # capture the environment at definition time
                    def __call__(self, runtime: 'MiniRuntime', args: List[Any]) -> Any: 
                        if len(args) != len(self.params):
                            raise TypeError(f"Expected {len(self.params)} args, got {len(args)}")

                        runtime.push_env(self.env + [{}])  # new frame with captured env
                        for p, v in zip(self.params, args):
                            runtime.set_var(p, v)
                            ret = None
                            for stmt in self.body:
                                ret = runtime.eval(stmt)
                                runtime.pop_env()
                                return ret
                            def __call__(self, runtime: 'MiniRuntime', args: List[Any]) -> Any:
                                if len(args) != len(self.params):
...
class Closure:
    ...
    vm = FastVM(bytecode_funcs, hot_registry)
print(vm.run_dodecagram("(+:1a:(*:3:4))"))        # → 34 (in base10)
print(vm.run_dodecagram('call(print:"Hello")'))  # prints "Hello"
print(vm.run_dodecagram("(if(>:(2):7:9))"))      # conditional example

def __init__(self, func: FuncDef, parent: Optional['Closure']=None):
        self.func = func
        self.parent = parent
        self.locals: Dict[str, Any] = {}
        self.children: List['Closure'] = []
        if parent:
            ...
            def get_var(self, name: str) -> Any:
                def get_var(self, name: str) -> Any:
                    ...
                    self.get_var = get_var.__get__(self)
